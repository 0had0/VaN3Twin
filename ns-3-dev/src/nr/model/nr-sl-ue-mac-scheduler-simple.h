...
private:
  ...
  /**
   * \brief Write PRNG state to scheduler CSV log file
   * \param sfnNorm Normalized SfnSf of the current slot
   * \param event Description of the event (e.g., "slot_pick", "sbch_pick")
   */
  void WriteSchedulerPrngStateDump (uint64_t sfnNorm, const std::string& event);

  bool m_enableSchedLog{false};       //!< Enable scheduler PRNG logging
  std::string m_schedLogDir;          //!< Directory for scheduler log files
  bool m_schedPrngFirstWrite{true};   //!< First write flag for CSV header
  uint16_t m_schedImsi{0};            //!< IMSI for file naming (set via attribute)
};
