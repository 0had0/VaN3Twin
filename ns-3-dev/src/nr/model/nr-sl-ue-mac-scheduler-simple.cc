...
NrSlUeMacSchedulerSimple::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrSlUeMacSchedulerSimple")
    ...
    .SetGroupName ("nr")
    .AddAttribute ("EnableSchedPrngLog",
                   "If true, write per-vehicle scheduler PRNG state CSV files",
                   BooleanValue (false),
                   MakeBooleanAccessor (&NrSlUeMacSchedulerSimple::m_enableSchedLog),
                   MakeBooleanChecker ())
    .AddAttribute ("SchedPrngLogDir",
                   "Directory prefix for scheduler PRNG log files",
                   StringValue (""),
                   MakeStringAccessor (&NrSlUeMacSchedulerSimple::m_schedLogDir),
                   MakeStringChecker ())
    .AddAttribute ("SchedLogImsi",
                   "IMSI for scheduler log file naming",
                   UintegerValue (0),
                   MakeUintegerAccessor (&NrSlUeMacSchedulerSimple::m_schedImsi),
      ...
      while (newTxOpps.size () != totalTx)
        {
          ...
          if (m_enableSchedLog)
            {
              WriteSchedulerPrngStateDump (txOpps.begin ()->sfn.Normalize (), "before_slot_pick");
            }
          // Walk through list until the random element is reached
          std::advance (txOppsIt, m_uniformVariable->GetInteger (0, txOpps.size () - 1));
          if (m_enableSchedLog)
            {
              WriteSchedulerPrngStateDump (txOppsIt->sfn.Normalize (), "after_slot_pick");
            }
      ...
      if (minContgSbCh == GetTotalSubCh() && assignedSbCh == 1)
        {
          ...
          if (m_enableSchedLog)
            {
              WriteSchedulerPrngStateDump (0, "before_sbch_pick");
            }
          uint8_t randIndex = static_cast<uint8_t> (m_uniformVariable->GetInteger (0, GetTotalSubCh() - 1));
          if (m_enableSchedLog)
            {
              WriteSchedulerPrngStateDump (0, "after_sbch_pick");
            }
...
void
NrSlUeMacSchedulerSimple::WriteSchedulerPrngStateDump (uint64_t sfnNorm, const std::string& event)
{
  std::ostringstream fname;
  fname << m_schedLogDir << "sched_prng_imsi_" << m_schedImsi << ".csv";
  std::ofstream ofs (fname.str (), std::ios::app);
  if (m_schedPrngFirstWrite)
    {
      ofs << "timestamp_ms,sfn_normalized,event,state_0,state_1,state_2,state_3,state_4,state_5" << std::endl;
      m_schedPrngFirstWrite = false;
    }
  double state[6];
  m_uniformVariable->GetRngState (state);
  ofs << Simulator::Now ().GetMilliSeconds ()
      << "," << sfnNorm
      << "," << event;
  for (int i = 0; i < 6; i++)
    {
      ofs << "," << std::setprecision (15) << state[i];
    }
  ofs << std::endl;
  ofs.close ();
}
