    ...
    static TypeId tid =
        TypeId("ns3::NrSlUeMac")
            ...
            .AddAttribute("EnableSpsSelectionLog",
                          ...
                          BooleanValue(false),
                          MakeBooleanAccessor(&NrSlUeMac::m_enableSpsLog),
                          MakeBooleanChecker())
            .AddAttribute("SpsSelectionLogDir",
                          "Directory prefix for SPS selection log files",
                          StringValue(""),
                          MakeStringAccessor(&NrSlUeMac::m_spsLogDir),
                          MakeStringChecker())
...
void
NrSlUeMac::WriteSpsSelectionRow(uint64_t sfnNorm, bool resourceReused, uint64_t selectedSlotNorm)
{
    if (!m_enableSpsLog)
    {
        return;
...
void
NrSlUeMac::WriteSensingWindowDump(uint64_t sfnNorm)
{
    if (!m_enableSpsLog)
    {
        return;
...
void
NrSlUeMac::WritePrngStateDump(uint64_t sfnNorm, const std::string& event)
{
    if (!m_enableSpsLog)
    {
        return;
